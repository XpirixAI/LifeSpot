...
<form method="POST" action="{{ route('myestate.financials.store.bank') }}" enctype="multipart/form-data">
    @csrf
        ...
        <div class="col-span-3 lg:col-span-3 space-y-3">
            <div class="lg:flex lg:space-x-2 space-y-4 lg:space-y-0">
                <div class="lg:w-full">
                    ...
                    <input type="text" name="bank_name"  autocomplete="bank_name" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md" value="{{ old('bank_name') }}" placeholder="Financial Institution">
                    @error('bank_name')
                        <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="pt-8">
                <a href="{{ route('myestate.financials.banking') }}" class="border border-gray-400 rounded-lg float-right font-semibold text-xs py-2 px-4">Cancel</a>
