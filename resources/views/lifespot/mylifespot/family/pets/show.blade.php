...
<!--Display Pet's Core Information -->
<div class="grid grid-cols-1 lg:grid-cols-5 lg:gap-x-2 gap-y-3 lg:grid-flow-row-dense mx-1 lg:mx-4">
    <div class="xbg-red-500 rounded-lg xshadow-xl min-h-20 row-span-6">
        <a class="flex text-sm text-blue-800 font-semibold mb-4" href="{{ route('mylifespot.family.pets') }}">
        ...
        <div class="flex space-x-8 mb-2">
                ...
                <h1 class="font-black text-xl">
                   ...
                   <a href="{{ route('mylifespot.family.edit.pet',$asset->id) }}" class="flex">
        ...
        <div class="grid grid-cols-7 gap-2 ">
          ...
          <div class="col-span-5 xspace-y-3 ml-6 lg:ml-0">
           ...
           <div class="space-y-3.5">
            <div class="text-gray-500">{{ ($asset->description) != NULL || '' ? $asset->description : '-' }}</div>
            {{-- <div class="text-gray-500 whitespace-nowrap">{{ ($asset->emergency_contact) != NULL || '' ? $asset->emergency_contact : '-' }}</div> --}}
...
<div class="mx-4">
    ...
    <div class="items-center space-y-3">
        ...
        <livewire:pet-emergencycontact :asset="$asset">
        ...
        <div class="mt-2 space-y-3 col-span-5 items-center lg:text-base text-sm">
            <div class="xbg-gray-400 grid grid-cols-8 lg:grid-cols-12 gap-0 lg:gap-4 items-center whitespace-nowrap">
                <div class="xbg-green-400 col-span-3 lg:col-span-3 text-right space-y-3">
                    ...
                    <p class="text-gray-800"><span class="hidden lg:inline">Emergency</span>  Contact
                        @isset($er_contact[0])
                            <button type="button" data-bs-toggle="modal" data-bs-target="#viewERContactModal" wire:model=$er_contact[0] class="bg-sky-800 text-white text-xs rounded-md  px-1 py-1">View</button>
                            @else
                            <button wire:click="create()" type="button" data-bs-toggle="modal" data-bs-target="#addERContact" class="bg-sky-800 text-white text-xs rounded-md  px-1.5 py-1">Add</button>
                        @endisset
                        :
                    </p>
                ...
                <div class="xbg-blue-400 col-span-5 lg:col-span-7 text-left ml-1 space-y-4 lg:space-y-3">
                    ...
                    <p class="text-gray-500">
                        @isset($er_contact[0])
                        <span wire:model="create()"> {{ $er_contact[0]->fname }} {{ $er_contact[0]->lname }} </span>
                            @else <span>-</span>
                        @endisset
                    </p>
        ...
        <div x-data="{open: false}" >
            <form method="POST" action="{{ route('store.multi.image.pet',$asset->id) }}" enctype="multipart/form-data" multiple="" class="border-none">
                @csrf
        ...
        <div class="ml-5">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-6 gap-4">
                ...
                <!-- Modal toggle -->
                    ...
                    <form method="POST" action="{{ route('destroy.mulit.image.pet',$additionalAsset->id)  }}">
                        @csrf
                        @method('DELETE')
