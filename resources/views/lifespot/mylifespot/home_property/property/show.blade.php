...
<!--Display (Property) Asset's Core Information -->
<div class="grid grid-cols-1 lg:grid-cols-5 lg:gap-x-2 gap-y-3 lg:grid-flow-row-dense mx-1 lg:mx-4">
    ...
    <div class="xbg-orange-500 rounded-lg xshadow-xl min-h-20 row-span-5 col-span-3 mt-5">
        <div class="grid grid-cols-7 gap-2 ">
            <div class="col-span-2 text-right mr-1 xspace-y-3">
                <div class="mb-3 ">Asset <span class="hidden lg:inline"> Name</span>:</div>
                <div class="">
                    <div class=""><span class="hidden lg:inline"> Asset</span> Address:</div>
                    <div class="mb-3 invisible">Address2:</div>
                </div>
                <div class="space-y-3.5 mr-1">
                    <div class="">Description:</div>
                    <div class="">Value <span class="hidden lg:inline"> Estimate</span>:</div>
                    <div class="">Notes:</div>
                </div>
            </div>
          {{-- <div class="col-span-3 text-right mr-1 xspace-y-3">
            <div class="mb-3 ">Property Name:</div>
            <div class="">
                <div class="mb-3">Property Address:</div>
                <div class="hidden mb-3 invisible">Address:</div>
            </div>
            <div class="space-y-3.5 mr-1">
                <div class="">Value Estimate:</div>
            </div>
          </div> --}}
          <div class="col-span-4 xspace-y-3 ml-6 lg:ml-0">
            <div class="mb-3 text-gray-500 whitespace-nowrap">{{ Str::limit($asset->name, 25, '...') }}</div>
            ...
            <div class="text-gray-500 mt-0 mb-3 whitespace-nowrap"> {{ ($asset->city ) != NULL || '' ? $asset->city .',': '-' }}
                ...
                {{ ($asset->zip ) != NULL || '' ? $asset->zip : '-' }}
            </div>
            <div class="text-gray-500 whitespace-nowrap">
                {{ ($asset->location ) != NULL || '' ? $asset->location : '-' }}
                {{-- {{ ($asset->address2 ) != NULL || '' ?  $asset->address2 : '-' }} --}}
            </div>
            <div class="mb-3 mt-4 text-gray-500 whitespace-nowrap">{{ Str::limit($asset->value, 25, '...') }}</div>
           ...
           {{-- <div class="space-y-3.5 mb-3"> --}}
            {{-- <div class="text-gray-500 ">${{ ($asset->value) != NULL || '' ? $asset->value : '-' }}</div> --}}
    ...
    <div class="xbg-yellow-500 rounded-lg min-h-56 row-span-4">
        <div class="h-72 overflow-y-auto bg-blue-50 rounded-xl p-6">
            ...
            <p class="mt-2 text-xs">Adding assets and records to your estate is important because it ensures that your wishes and financial matters are taken care of after your death. Lifespot makes it easy to organize these documents and photos within the Document Center. The Document Center is a secure, cloud-based storage system that allows users to store and organize their important documents and photos. With Lifespot, users can create folders, add tags and search for documents easily. Lifespot also allows users to share documents securely with family members and other trusted advisors.</p>
        </div>
    </div>
...
<div class="mx-4">
    ...
    <div class="items-center space-y-3">
        <h3 class="font-semibold text-lg">Additional Information Related To <span class="">{{ Str::limit($asset->name, 25, '...') }}</span> </h3>

        <livewire:creditor :asset="$asset">
        ...
        <div class="mt-2 space-y-3 col-span-5 items-center lg:text-base text-sm">
            <div class="xbg-gray-400 grid grid-cols-8 lg:grid-cols-12 gap-0 lg:gap-4 items-center whitespace-nowrap">
                <div class="xbg-green-400 col-span-3 lg:col-span-3 text-right space-y-3">
                    <p class="text-gray-800">Lien(s)

                        @isset($creditor[0])
                            <button  type="button" data-bs-toggle="modal" data-bs-target="#viewCreditorModal" wire:model=$creditor[0] class="bg-sky-800  text-white text-xs rounded-md  px-1 py-1">View</button>
                            @else
                            <button  wire:click="create()" type="button" data-bs-toggle="modal" data-bs-target="#addCreditor" class="bg-sky-800 text-white text-xs rounded-md  px-1.5 py-1">Add</button>
                        @endisset
                        :
                    </p>
                </div>
                <div class="xbg-blue-400 col-span-5 lg:col-span-7 text-left ml-1 space-y-4 lg:space-y-3">
                    <p class="text-gray-500">
                        @isset($creditor[0])
                            ...
                            <span wire:model="create()"> {{ $creditor[0]->creditor_name }} </span>
                            @else <span>-</span>
                        @endisset
                    </p>
                </div>
