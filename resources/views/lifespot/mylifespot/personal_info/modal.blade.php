    ...
    <div x-cloak x-show="showModal" style="background-color: rgba(0, 0, 0, .5)"
                ...
                <div class="grid grid-cols-2 gap-4 pt-5 pb-2">
                    <div>
                        ...
                        <div class="xlg:w-1/2 text-xs lg:text-sm">
                            <label for="title" class="block  font-black text-black">Birth Certificate</label>
                            <input type="text" name="title"  autocomplete="title" class="mt-1 text-gray-700 focus:ring-blue-500 focus:border-blue-500 block w-full text-xs lg:text-sm border-gray-300 rounded-md" value="{{ old('title') }}" placeholder="Birth Certificate">
                            @error('title')
                    ...
                    <div>
                        ...
                        <div class="float-right mt-5 space-y-2">
                            <button @click="showModal = false" class="border border-blue-800 rounded-lg font-semibold text-xs py-2 px-8 mr-1 text-[#056591]">Cancel</button>
        ...
        <!-- Modal Top Right Demo-->
        <div x-cloak x-show="showTopRightDemoModal" style="background-color: rgba(0, 0, 0, .5)"
        class="fixed inset-0 z-30 flex items-center justify-center overflow-auto bg-black bg-opacity-50">
        <!-- inner modal -->
        <div
            @click.away="showTopRightDemoModal = false"
            x-transition:enter="motion-safe:ease-out duration-500"
            x-transition:enter-start="opacity-0 scale-90"
            x-transition:enter-end="opacity-100 scale-100"
            class="xmax-w-3xl px-5 py-4 mx-auto text-left bg-white rounded-xl shadow-lg">
            ...
            <div class="flex items-center justify-between space-y-3">
                <h5 class="mr-3 font-black text-black xmax-w-none">Upload a Document</h5>
                <button type="button" class="z-50 cursor-pointer" @click="showTopRightDemoModal = false">
                    <svg xmlns="http://www.w3.org/2000/svg" xwidth="20" xheight="20 text-gray-300" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
            ...
            <img class="mt-2" src="{{ asset('img/WalkerFamilyPics/drag_drop.png') }}" alt="">
                ...
                <div class="grid grid-cols-2 gap-4 pt-5 pb-2">
                    <div>
                        <div class="font-semibold text-gray-700 text-xs lg:text-sm mb-2">Documnet Title</div>
                        <div class="xlg:w-1/2 text-xs lg:text-sm">
                            <label for="title" class="block  font-black text-black">Social Secutrity Card</label>
                            <input type="text" name="title"  autocomplete="title" class="mt-1 text-gray-700 focus:ring-blue-500 focus:border-blue-500 block w-full text-xs lg:text-sm border-gray-300 rounded-md" value="{{ old('title') }}" placeholder="Social Security Card">
                            @error('title')
                                <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <div class="float-right mt-5 space-y-2">
                            <button @click="showTopRightDemoModal = false" class="border border-blue-800 rounded-lg font-semibold text-xs py-2 px-8 mr-1 text-[#056591]">Cancel</button>
                            <button class="bg-[#1f588d] text-white border border-gray-400 rounded-lg font-semibold text-xs py-2 px-8">Upload</button>
                        </div>
